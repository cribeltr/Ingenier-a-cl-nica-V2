...
VIEWS.asignaciones = function(root, params){
  ...
  // Filtro "drill-down" que llega desde el Resumen (estado de la MP / sin ejecutar).
  ...
  const COLS = [
    ...
    {k:'ejecutor', l:'Ejecutor',     g:r=>r.ejecutor||'', lista:true},
  ...
  function buildRows(){
    ...
    state.equipos.forEach(eq=>{
      ...
      meses.forEach(m=>{
        ...
        rows.push({eq, m, P, R, ejecutor: ev?(ev.ejecutor||''):'', estadoReg: ev?(ev.oficial==='Sí'?'Oficial':'Borrador'):''});
      });
    });
    return rows;
  }
  ...
  function renderChips(){
    ...
    if(drill.sinAsignar) addChip('Resumen','Sin ejecutar',()=>{drill.sinAsignar=false;renderChips();aplicar();});
  }
  function aplicar(){
    const tokens=norm(search.value.trim()).split(/\s+/).filter(Boolean);
  ...
  function render(){ rowsAll = buildRows(); buildHead(); aplicar(); }
  ...
  root.appendChild(el('div',{class:'view view-full'},
    el('div',{style:{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'8px',flexWrap:'wrap',gap:'10px'}},
      el('h2',{},'Asignaciones'),
      el('div',{style:{display:'flex',gap:'6px',alignItems:'center'}}, el('span',{class:'muted',style:{fontSize:'12px'}},'Año'), selAnio, el('span',{class:'muted',style:{fontSize:'12px'}},'Mes'), selMes)
    ),
    el('div',{class:'subtitle'},'Programación, resultado y ejecutor de la MP por equipo y mes. Filtra por columna o elige el mes (incluye "Todos los meses").'),
