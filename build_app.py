...
v0.49 [2026-05-29] Rediseño minimalista (paso 1): paleta azul, densidad compacta, sidebar contraíble.
  ...
  - Barra lateral oscura ahora CONTRAÍBLE: botón ☰ en la barra superior la pliega/expande y se
    recuerda la preferencia (sidebarColapsada).
  - Tablas más limpias (hover de fila, encabezados compactos), tarjetas/secciones con esquinas
    más suaves y sombra sutil, botones con radio 6px y foco accesible.
...
v0.41 [2026-05-29] Ajustes pedidos tras la primera sesión de uso real.
  - Buscar equipos: hacer CLIC EN LA FILA abre la ficha (se quita el botón "Ficha"). Los
    ...
    desplazamiento lateral (en la sesión el usuario se trababa al estirarse al botón).
  - MP con causal C1–C8: además del pendiente de reprogramación, marca de inmediato una "R"
    (reprogramado) en la PROGRAMACIÓN del MES SIGUIENTE de la carta gantt (registro[mesSig].P).
    No pisa una programación ya puesta. La ficha lee P desde registro.P (como la planilla).
  - Historial de eventos: nuevo botón "🖨 Imprimir" que abre una hoja limpia con la hoja de
    vida del equipo y lanza el diálogo de impresión (o guardar como PDF), 100% offline.
  - Verificado que al subir el maestro los resultados R de la gantt aparecen en el historial
    como eventos "🔗 Auto-maestro" (ya ocurría desde v0.21; confirmado con prueba).
...
<style>
:root{
  /* Tema CLARO minimalista — neutro frío, acento azul */
  --bg:#f5f6f8;
  --surface:#ffffff;
  --surface-2:#eef1f5;
  --border:#e6e9ee;
...
[data-theme="dark"]{
  /* Tema OSCURO — gris-azul profundo, acento azul */
  --bg:#0f1216;
  --surface:#161a20;
  --surface-2:#1e232b;
  --border:#272d36;
...
html,body{margin:0;padding:0;height:100%;font-family:var(--font);font-size:13.5px;color:var(--text);background:var(--bg);-webkit-font-smoothing:antialiased;font-feature-settings:"cv11","ss01";line-height:1.45;transition:background-color .2s,color .2s}
...
button{font-family:inherit;font-size:inherit;cursor:pointer;border:1px solid var(--border-strong);background:var(--surface);color:var(--text);padding:5px 11px;border-radius:6px;transition:background .12s,border-color .12s,color .12s,box-shadow .12s;font-weight:500}
...
input,select,textarea{font-family:inherit;font-size:inherit;padding:6px 9px;border:1px solid var(--border-strong);border-radius:6px;background:var(--surface);color:var(--text);width:100%}
...
.app{display:grid;grid-template-columns:230px 1fr;height:100vh;overflow:hidden;background:var(--bg);transition:grid-template-columns .18s ease}
.app.sb-collapsed{grid-template-columns:0 1fr}
/* Barra lateral oscura, contraíble */
.sidebar{background:#0f172a;color:#cbd5e1;display:flex;flex-direction:column;overflow-y:auto;overflow-x:hidden;white-space:nowrap}
...
.topbar{display:flex;align-items:center;gap:6px;padding:0 14px;background:var(--surface);border-bottom:1px solid var(--border)}
.topbar button{font-size:12.5px;padding:5px 10px;color:var(--muted);border:1px solid var(--border);border-radius:6px;background:var(--surface)}
.topbar button:hover{color:var(--text);background:var(--surface-2)}
.sb-toggle{border:none!important;background:transparent!important;color:var(--muted);font-size:17px!important;padding:4px 9px!important;border-radius:6px;line-height:1}
.sb-toggle:hover{background:var(--surface-2)!important;color:var(--text)}
...
table.data th{background:var(--surface-2);text-align:left;padding:7px 10px;font-weight:600;border-bottom:1px solid var(--border);color:var(--muted);white-space:nowrap;position:sticky;top:0;z-index:1;font-size:11px;text-transform:uppercase;letter-spacing:.03em}
...
/* Section card */
.section{background:var(--surface);border:1px solid var(--border);border-radius:10px;padding:15px;margin-bottom:15px;box-shadow:var(--shadow-sm)}
...
<div class="app">
  ...
  <div class="content">
    <header class="topbar">
      <button id="btn-sidebar" class="sb-toggle" title="Plegar / expandir el menú lateral">☰</button>
...
// Evento MP (no anulado) de un equipo en un mes/año dado. Sirve para ejecutor y estado del registro.
...
function aplicarEfectosEvento(ev){
  ...
  if(tipo === 'Mantención preventiva'){
    ...
    if(r && /^C[1-8]$/.test(r)){
      ...
      // Una causal reprograma la MP: marca "R" (reprogramado) en la programación del MES
      // SIGUIENTE para que se vea de inmediato en la carta gantt. No pisa una programación
      // ya existente de ese mes (solo escribe si está vacía).
      if(mesIdx < 11){
...
function toggleSidebar(){
  ...
  const colapsada = app.classList.toggle('sb-collapsed');
  setPref('sidebarColapsada', colapsada);
  save({internal:true});
}
...
function renderSumMesesMP(year, returnParts){
  ...
  function c(val, mIdx, params){
    if(val === 0) return el('td',{class:'cell zero'}, '–');
    return el('td',{class:'cell',onclick:()=>navigate('asignaciones',{mes:MESES[mIdx], year, ...params})}, el('span',{class:'v'}, String(val)));
  }
  const table = el('table',{class:'sum-table'},
    ...
    el('tbody',{},
      ...rows.map(r => el('tr',{},
        ...
        r.sinAsignar > 0 ? el('td',{class:'cell',onclick:()=>navigate('asignaciones',{mes:r.mes, year, sinAsignar:'1'}),style:{color:r.sinAsignar>0?'var(--st)':null}}, el('span',{class:'v'}, String(r.sinAsignar))) : el('td',{class:'cell zero'},'–'),
...
function renderSumEjecutoresMP(year, monthIdx, returnParts){
  ...
  function c(val, ejecutor, params){
    if(val === 0) return el('td',{class:'cell zero'}, '–');
    return el('td',{class:'cell', onclick:()=>navigate('asignaciones', {ejecutor, mes:NUM_MES[monthIdx], year, ...params})}, el('span',{class:'v'}, String(val)));
  }
  ...
  const table = el('table',{class:'sum-table'},
    ...
    el('tbody',{},
      ...
      sinAsignar > 0 ? el('tr',{style:{background:'#fef9c3'}},
        ...
        el('td',{class:'cell', onclick:()=>navigate('asignaciones',{mes:NUM_MES[monthIdx], year, sinAsignar:'1'})}, el('span',{class:'v'}, String(sinAsignar))),
        el('td',{}, '–'), el('td',{}, '–'), el('td',{}, '–')
      ) : null,
      el('tr',{class:'total-row'},
        el('td',{},'Total'),
...
function renderMatrizMP(eq){
  ...
  MESES.forEach((m, idx) => {
    const p = ((eq.registro||{})[m]||{}).P || (eq.prog||{})[m] || '';
...
// Imprime el historial de eventos del equipo: abre una ventana limpia y lanza el diálogo
// de impresión del navegador (también permite "Guardar como PDF"). 100% offline.
function imprimirHistorial(eq){
  ...
  const filas = evs.map(ev => {
    ...
    const desc = esc(etiquetaTipoEvento(ev))
      ...
      + (ev.anulado ? ' · ANULADO' : '');
    return `<tr class="${ev.anulado?'anu':''}"><td>${esc(fmtFecha(ev.fecha))}</td><td>${desc}</td>`
      + `<td>${esc(ev.estado||'')}</td><td>${esc(ev.ejecutor||'')}</td>`
      + `<td>${esc(ev.obs||'')}</td><td>${pendAsoc>0?pendAsoc:''}</td></tr>`;
  }).join('');
  const html = '<!doctype html><html lang="es"><head><meta charset="utf-8">'
    + '<title>Historial '+esc(eq.inv)+'</title><style>'
    + 'body{font-family:Arial,Helvetica,sans-serif;font-size:12px;color:#111;padding:24px}'
    + 'h1{font-size:16px;margin:0 0 4px} h2{font-size:12px;font-weight:400;color:#555;margin:0 0 14px}'
    + 'table{border-collapse:collapse;width:100%} th,td{border:1px solid #999;padding:5px 7px;text-align:left;vertical-align:top}'
    + 'th{background:#eee} tr.anu td{color:#999;text-decoration:line-through}'
    + '.tb{margin:10px 0 16px} @media print{.noprint{display:none}}'
    + '</style></head><body>'
    + '<h1>Historial de eventos — '+esc(eq.equipo||'')+' · '+esc(eq.inv)+'</h1>'
    + '<h2>'+esc(fmt(eq.marca))+' '+esc(fmt(eq.modelo))+' · Serie '+esc(fmt(eq.serie))
    +   ' · '+esc(fmt(eq.servicio))+' · Estado: '+esc(ESTADO_LABEL[eq.estado]||eq.estado||'—')
    +   ' · Impreso '+esc(new Date().toLocaleString('es-CL'))+'</h2>'
    + '<div class="tb noprint"><button onclick="window.print()">🖨 Imprimir</button></div>'
    + '<table><thead><tr><th>Fecha</th><th>Descripción</th><th>Estado</th><th>Ejecutor</th><th>Observación</th><th>Pend.</th></tr></thead>'
    + '<tbody>'+(filas||'<tr><td colspan="6">Sin eventos registrados.</td></tr>')+'</tbody></table>'
    + '</body></html>';
  const w = window.open('', '_blank');
  if(!w){ toast('Permite las ventanas emergentes del navegador para imprimir.', 'error'); return; }
...
//---------------- ASIGNACIONES (consulta de MP por mes) ----------------
VIEWS.asignaciones = function(root, params){
  params = params || {};
  const today = new Date();
  const norm = normFiltro;
  const filtros = {};
  if(params.ejecutor) filtros.ejecutor = new Set([params.ejecutor]);
  // Filtro "drill-down" que llega desde el Resumen (estado de la MP / sin asignar).
  const drill = { estadoMP: params.estadoMP || '', sinAsignar: !!params.sinAsignar };
  let ordK=null, ordDir=1, rowsAll=[];
  ...
  selAnio.value = (params.year && ANIOS.includes(+params.year)) ? +params.year : today.getFullYear();
  const selMes = el('select',{}, el('option',{value:'todos'},'Todos los meses'), ...MESES.map((m,i)=>el('option',{value:i}, m)));
  selMes.value = (params.mes!=null && MES_NUM[params.mes]!=null) ? String(MES_NUM[params.mes]) : String(today.getMonth());
  const search = el('input',{type:'search',placeholder:'Buscar por equipo, N° inventario, servicio…'});
  const thead=el('thead',{}); const tbody=el('tbody',{});
  const counter=el('div',{class:'muted',style:{fontSize:'12px',whiteSpace:'nowrap'}},'');
  const chipsBar=el('div',{class:'filters'});

  // Plantillas (requieren un mes específico; se deshabilitan con "Todos los meses").
  const btnDescPlantilla = el('button',{onclick:()=>descargarPlantillaMP(+selAnio.value, +selMes.value)},'📥 Descargar plantilla');
  const fileInputPlantilla = el('input',{type:'file',accept:'.xlsx',style:{display:'none'},onchange:async e=>{ const f=e.target.files[0]; if(!f) return; await subirPlantillaMP(f, +selAnio.value, +selMes.value); e.target.value=''; render(); }});
  const btnSubirPlantilla = el('button',{class:'primary',onclick:()=>fileInputPlantilla.click()},'📤 Subir plantilla');
  function syncPlantilla(){ const todos = selMes.value==='todos'; btnDescPlantilla.disabled=todos; btnSubirPlantilla.disabled=todos;
    const t = todos?'Elige un mes específico para usar la plantilla':''; btnDescPlantilla.title=t||'Descargar plantilla Excel del mes'; btnSubirPlantilla.title=t||'Subir plantilla con las asignaciones llenadas'; }

  function asignarEjecutor(keyMes, inv, ejec){
    state.asignacionesMP[keyMes] = state.asignacionesMP[keyMes] || {};
  ...
  const COLS = [
    {k:'mes',      l:'Mes',          g:r=>MESES[r.m], lista:true},
    {k:'carpeta',  l:'N° Carpeta',   g:r=> r.eq.carpeta!=null?String(r.eq.carpeta):''},
    {k:'inv',      l:'N° Inventario',g:r=>r.eq.inv||'', cell:r=>el('td',{}, el('strong',{}, r.eq.inv||'—'))},
    {k:'serie',    l:'Serie',        g:r=>r.eq.serie||''},
    {k:'equipo',   l:'Equipo',       g:r=>r.eq.equipo||'', lista:true},
    {k:'marca',    l:'Marca',        g:r=>r.eq.marca||'', lista:true},
    {k:'modelo',   l:'Modelo',       g:r=>r.eq.modelo||'', lista:true},
    {k:'servicio', l:'Servicio',     g:r=>r.eq.servicio||'', lista:true},
    {k:'unidad',   l:'Unidad',       g:r=>r.eq.unidad||'', lista:true},
    {k:'ubic',     l:'Ubicación',    g:r=>r.eq.ubic||'', lista:true},
    {k:'proc',     l:'Procedencia',  g:r=>r.eq.proc||'', lista:true},
    {k:'prog',     l:'Programación', g:r=>r.P||'', lista:true},
    {k:'resultado',l:'Resultado',    g:r=>r.R||'', lista:true},
    {k:'ejecutor', l:'Ejecutor (asignado)', g:r=>r.ejecutor||'', lista:true, cell:r=>{
      const sel = el('select',{onchange:e=>{ asignarEjecutor(r.keyMes, r.eq.inv, e.target.value); r.ejecutor = e.target.value||''; }},
        el('option',{value:''},'— sin asignar —'), ...EJECUTORES.map(x=>el('option',{value:x},x)));
      sel.value = r.ejecutor||''; return el('td',{}, sel);
    }},
    {k:'estadoReg',l:'Estado registro', g:r=>r.estadoReg||'', lista:true, cell:r=>el('td',{}, r.estadoReg? el('span',{class:'badge '+(r.estadoReg==='Oficial'?'ofic-si':'ofic-no')}, r.estadoReg) : el('small',{class:'muted'},'—'))}
  ];

  function buildRows(){
    state.equipos.forEach(recalcEstadoEquipo);
    const yy = +selAnio.value;
    const meses = selMes.value==='todos' ? MESES.map((_,i)=>i) : [ +selMes.value ];
    const rows=[];
    state.equipos.forEach(eq=>{
      if(eq.estado==='baja') return;
      meses.forEach(m=>{
        const P = ((eq.registro||{})[MESES[m]]||{}).P || (eq.prog||{})[MESES[m]] || '';
        const R = resultadoMPMes(eq, yy, m) || '';
        if(!P && !R) return; // solo filas con programación o resultado en ese mes
        const keyMes = `${yy}-${String(m+1).padStart(2,'0')}`;
        const ev = eventoMPMes(eq.inv, yy, m);
        rows.push({eq, m, keyMes, P, R, ejecutor: (state.asignacionesMP[keyMes]||{})[eq.inv] || '', estadoReg: ev?(ev.oficial==='Sí'?'Oficial':'Borrador'):''});
      });
    });
    return rows;
  }
  const valoresUnicos = c => [...new Set(rowsAll.map(r=>{const v=c.g(r); return v==null?'':String(v);}))].filter(v=>v!=='').sort((a,b)=>a.localeCompare(b,'es',{numeric:true}));

  function buildHead(){
    cerrarColPopups();
    thead.innerHTML='';
    const trH=el('tr',{});
    COLS.forEach(c=> trH.appendChild(el('th',{class:'th-sort',title:'Ordenar por '+c.l,onclick:()=>{ if(ordK===c.k) ordDir=-ordDir; else {ordK=c.k;ordDir=1;} buildHead(); aplicar(); }}, c.l+(ordK===c.k?(ordDir>0?' ▲':' ▼'):''))));
    ...
    COLS.forEach(c=>{
      ...
      else ctrl=el('input',{type:'text',placeholder:'filtrar…',value:(typeof filtros[c.k]==='string'?filtros[c.k]:''),oninput:e=>{ filtros[c.k]=e.target.value; clearTimeout(window.__af); window.__af=setTimeout(aplicar,200); }});
      trF.appendChild(el('th',{}, ctrl));
    });
  ...
  function renderChips(){
    chipsBar.innerHTML='';
    const etq={ejec:'Ejecutadas',pend:'Pendientes',reprog:'Reprogramadas (C1–C8)',otro:'Otro resultado'}[drill.estadoMP];
    const addChip=(k,v,onclr)=>chipsBar.appendChild(el('span',{class:'filter-chip'}, el('span',{class:'k'},k+':'), el('span',{},v), el('span',{class:'x',onclick:onclr},'×')));
    if(etq) addChip('Resumen', etq, ()=>{drill.estadoMP='';renderChips();aplicar();});
    if(drill.sinAsignar) addChip('Resumen','Sin asignar',()=>{drill.sinAsignar=false;renderChips();aplicar();});
  }
  function aplicar(){
    const tokens=norm(search.value.trim()).split(/\s+/).filter(Boolean);
    let list=rowsAll.filter(r=>{
      if(drill.estadoMP==='ejec' && r.R!=='Si') return false;
      if(drill.estadoMP==='pend' && r.R) return false;
      if(drill.estadoMP==='reprog' && !/^C[1-8]$/.test(r.R||'')) return false;
      if(drill.estadoMP==='otro' && (!r.R || r.R==='Si' || /^C[1-8]$/.test(r.R))) return false;
      if(drill.sinAsignar && r.ejecutor) return false;
      if(tokens.length){ const hay=[r.eq.inv,r.eq.serie,r.eq.equipo,r.eq.marca,r.eq.modelo,r.eq.servicio,r.eq.unidad].map(norm).join(' '); if(!tokens.every(t=>hay.includes(t))) return false; }
      for(const c of COLS){ const fv=filtros[c.k]; if(!fv) continue; const val=c.g(r);
        if(c.lista){ if(fv instanceof Set && fv.size>0 && !fv.has(String(val))) return false; }
        else { if(!norm(val).includes(norm(fv))) return false; } }
      return true;
    });
    ...
    tbody.innerHTML='';
    list.slice(0,1000).forEach(r=>{
      const tr=el('tr',{});
      COLS.forEach(c=> tr.appendChild(c.cell ? c.cell(r) : el('td',{}, c.g(r)||'—')));
      tr.appendChild(el('td',{class:'actions',style:{whiteSpace:'nowrap'}},
        el('button',{class:'small primary',onclick:()=>mpRapida({invDefault:r.eq.inv, fechaDefault:fechaSugeridaMP(+selAnio.value, r.m)})},'➕ MP'),
        el('button',{class:'small ghost',onclick:()=>navigate('equipo',{inv:r.eq.inv})},'Ficha')
      ));
      tbody.appendChild(tr);
    });
    counter.textContent = `${list.length} registros${list.length>1000?' (mostrando 1000)':''}`;
    renderChips();
  }
  function render(){ syncPlantilla(); rowsAll = buildRows(); buildHead(); aplicar(); }
  selAnio.onchange = render; selMes.onchange = render;
  search.addEventListener('input', ()=>{ clearTimeout(window.__as); window.__as=setTimeout(aplicar,200); });
  ...
  root.appendChild(el('div',{class:'view view-full'},
    el('div',{style:{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'8px',flexWrap:'wrap',gap:'10px'}},
      ...
      el('div',{style:{display:'flex',gap:'6px',alignItems:'center',flexWrap:'wrap'}},
        el('span',{class:'muted',style:{fontSize:'12px'}},'Año'), selAnio,
        el('span',{class:'muted',style:{fontSize:'12px'}},'Mes'), selMes,
        btnDescPlantilla, btnSubirPlantilla, fileInputPlantilla
      )
    ),
    el('div',{class:'subtitle'},'Programación, resultado y ejecutor ASIGNADO de la MP por equipo y mes. Asigna el ejecutor en su columna; descarga/sube la plantilla del mes; filtra por columna o por mes ("Todos los meses").'),
...
function bootstrap(){
  ...
  if(getPref('sidebarColapsada', false) === true) document.querySelector('.app').classList.add('sb-collapsed');
